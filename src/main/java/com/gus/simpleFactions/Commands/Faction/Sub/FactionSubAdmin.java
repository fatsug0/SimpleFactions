    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "claim":
                ...
                for (FactionObject factionObject : plugin.factionManager.existingFactions){
...
//                        if (Integer.parseInt(args[2]) )
    ...
    private void ForceClaim(FactionObject factionObject, Chunk chunkToClaim) {
        ...
        if (plugin.factionManager.linkedChunks.containsKey(chunkToClaim)) {
            ForceUnClaim(plugin.factionManager.linkedChunks.get(chunkToClaim), chunkToClaim);
        }

        boolean weakClaim = false;
        if (factionObject.getClaimedChunks().size() < factionObject.getPower()) {

            // Link Chunk to Faction
            factionObject.getClaimedChunks().add(chunkToClaim);

        } else if (factionObject.getWeakChunks().size() <= factionObject.getMaxWeakChunks()) {
            if (!plugin.getConfig().getBoolean("faction.object.weak-claims-enabled")) return;

            // Link Raidable Chunk to Faction
            factionObject.getWeakChunks().add(chunkToClaim);
            weakClaim = true;

        }

        // Add Chunk in FactionManager claimedChunkCache (claimed or raidable doesn't matter)
        plugin.factionManager.linkedChunks.put(chunkToClaim, factionObject);

        // Change player state
        for (Player player : Bukkit.getOnlinePlayers()){
            if (player.getLocation().getChunk().equals(chunkToClaim)){
                if (weakClaim){
                    plugin.factionManager.playerInProtectedChunks.put(player.getUniqueId(), PlayerChunkState.WEAK);
                } else {
                    plugin.factionManager.playerInProtectedChunks.put(player.getUniqueId(), PlayerChunkState.HARD);
                }
            }
        }
        ...
        if (plugin.getConfig().getBoolean("enable-bluemap-addon")) {
            ...
            plugin.factionManager.bluemapClaimedChunk.put(chunkToClaim, factionObject.markerSet.getMarkers().size());

            // Put the new ShapeMarker in the MarkerSet
            factionObject.markerSet.getMarkers().put("claimedLand " + factionObject.markerSet.getMarkers().size(), shapeMarker);
            ...
            BlueMapAPI.onEnable(api -> {
                api.getWorld(Bukkit.getWorld("world")).ifPresent(world -> {
                    for (BlueMapMap map : world.getMaps()) {
                        map.getMarkerSets().put("FactionMarkerSet" + factionObject.getFactionName() + " claims", factionObject.markerSet);
                    }
                });
            });
        }
        factionObject.drawFactionOutsideLine();
    }
    ...
    private void ForceUnClaim(FactionObject factionObject, Chunk chunkToCheck) {
        ...
        plugin.factionManager.linkedChunks.remove(chunkToCheck);

        // Change player state
        for (Player player : Bukkit.getOnlinePlayers()){
            if (player.getLocation().getChunk().equals(chunkToCheck)) {
                plugin.factionManager.playerInProtectedChunks.put(player.getUniqueId(), PlayerChunkState.WILDERNESS);
            }
        }

        // Unlink Chunk to Faction
        factionObject.getClaimedChunks().remove(chunkToCheck);

        if (plugin.getConfig().getBoolean("enable-bluemap-addon")) {

            // Remove the ShapeMarker from the MarkerSet
            factionObject.markerSet.remove("claimedLand " + plugin.factionManager.bluemapClaimedChunk.get(chunkToCheck));

            // Remove the ShapeMarker from the related Hashmap
            plugin.factionManager.bluemapClaimedChunk.remove(chunkToCheck);
